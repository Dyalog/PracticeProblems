:Namespace Test
    ...
    ∇ (ok report)←{class}(data Run)(n expr)
      ...
      :Else
          (ok report)←¯315 'Internal error – '
          report,←(Code a),' as test case ',Error ⎕EN
          :Return
      :EndTrap
      b←''
      :If 2=defs.⎕NC'b'
          ...
          :Else
              (ok report)←¯315 'Internal error – '
              report,←(Code b),' as edge case ',Error ⎕EN
              :Return
          :EndTrap
      :Else
          bs←⍬
      ...
      :For a :In as
          ...
          :Else
              ...
              report,←(Error ⎕EN),' with ',Code Args a
              :Return
          :EndTrap
...
⍝      resp←tag('apl-dyalog'##.Tio∘''Parse)script
      (ca cb)←≢¨as bs
      ...
      :ElseIf ∧/1=respa
          ...
          report,←' which should give ',Code defs.f On b
      :ElseIf ∨/0≥respa
          (a ok)←as respa⊃¨⍨ca⌊1⍳⍨1≠respa
          :If 0=⌊/ok
              report←'Result should have been ',Code defs.f On a
          :Else
    ...
    ∇
      ...
      Args←{
          val←-≢⍵
          ∊val↑' as left argument and ' ' as right argument',⍨∘⍺⍺¨⍵
      }
      Error←{
          ' caused ',⎕EM|⍵
      }
